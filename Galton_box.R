...
no.of.levels <- 14 # How many times a ball chooses to fall left or right
iterations <- 300 #or no of balls sent
width <- 600 #width and height of the animantion plot
height <- 500
# The location of your output folder where the animation is stored
filepath <- "C:\\Users\\AmolShah\\Desktop\\Galton_box" 
# ===========================================================================================
...
getTriangleXYcoors <- function(height.level,width.level,height.segment,width.segment){
  ...
  if(i%%2==1){
    for(j in 1:min(i,width.segments-i+1)){
      if(j%%2==1){
        ...
        polydata<- rbind(polydata,data.frame(x = unlist(coords[1:4]), y = unlist(coords[5:8]), 
                                             group = paste0(i,j),color="black", fill = "white") )
...
circleFun <- function(center = c(0,0),
                      group, 
                      iteration=1, diameter = width.segment/1.2, npoints = 16){
  r = diameter / 2
  tt <- seq(0,2*pi,length.out = npoints)
  xx <- center[1] + r * cos(tt)
  yy <- center[2] + r * sin(tt)
  return(data.frame(x = xx, y = yy,group=group,iteration=iteration))
}
...
breaks <- to_vec(for(i in 0:no.of.levels) i*(i+1)/2 )
level <- function(a){
...
iteration_circles <- data.table("circle1" = (rep(c(1,0),c(iterations-no.of.levels,no.of.levels))))

iteration_circles[ , paste0("circle",2:(no.of.circles+1)) := rep(0,iterations)]
coin <- c(0,1) # For random toss between falling left or falling right
for(i in 1:iterations){
  for(j in no.of.circles:1){
    level.j <- level(j)
    if(iteration_circles[i,..j] == 1 & level.j != no.of.levels){
      random_toss <- sample(coin, prob = c(0.5, 0.5), replace = TRUE,size=1)
      if(random_toss == 0){iteration_circles[i+1,j+level.j] <- 1}
      else{iteration_circles[i+1,j+level.j+1] <- 1}
    }
  }
}
...
final.circles.dt <- data.table(x=numeric(), y=numeric(), group=numeric(),iteration=numeric())
for(circle in 1:no.of.circles){
  center = circleCenter(circle)
  for(iter in 1:iterations){
    if(iteration_circles[iter,..circle] == 1){
      final.circles.dt <- rbind(final.circles.dt, circleFun(center, circle,iter))
    }
  }
}

# Data for the text labels and color of balls seen in the plot
text.x.coors <- seq(1,no.of.levels)*2*width.segment - 0.5*width.segment
text.y.coors <- seq(-height/20,-height/20,no.of.levels)
text.label <- seq(1,no.of.levels)
text.dt <- data.table(x=text.x.coors, y=text.y.coors, label=text.label)
colfunc<-colorRampPalette(c("red","yellow2","springgreen","royalblue","red"))
colors <- colfunc(1.1*iterations*no.of.circles)
final.circles.dt$color.circle <- colors[(final.circles.dt$iteration-1)*no.of.circles+final.circles.dt$group*no.of.circles/10] 
...
viz <- viz + 
  geom_polygon(data=final.circles.dt, 
               aes(x=x,y=y,
                   group=group,
                   color=color.circle,fill=color.circle),
                   showSelected ="iteration") + #, chunk_vars=c("iteration") - But this makes loading slow
...
col.nums <- (no.of.levels*(no.of.levels-1)/2+1):(no.of.levels*(no.of.levels+1)/2)
iteration_circles.hist <- cumsum(iteration_circles[, ..col.nums])
colnames(iteration_circles.hist) <- as.character(unlist(1:no.of.levels))
iteration_circles.hist <- cbind(iteration_circles.hist,iteration=rownames(iteration_circles.hist))
molten <- melt(iteration_circles.hist, id.vars="iteration",variable.name= "circleNum")
molten$iteration <- as.numeric((molten$iteration))

# Add text to bar-plot
max.count <- max(molten$value)
text_bar_coors <- seq(-0.05*max.count,-0.05*max.count,no.of.levels)
...
viz2 <- ggplot()+
  ...
  theme(
    ...
    panel.grid.major=element_blank(),
    panel.grid.minor=element_blank()) + 
  geom_bar(aes(x=circleNum, y=value,
               group=circleNum),
           data=molten,
           fill="white", color="black",
           showSelected="iteration",stat = "identity", 
           position="identity")+scale_colour_identity()+
  scale_fill_identity()+
  geom_text(data = text.dt, aes(x=label,y=text_bar_coors,label = label,group=label),size=18)+
  geom_text(data = molten, 
            aes(x=circleNum,y=value+0.035*max.count,
                label = value,group=circleNum),
            size=18,showSelected="iteration")

# Final animation
(viz.final2 <- animint(viz,
                       viz2,
